...
class AddBusiness extends Component {
    ...
    handleSubmit(e) {
        ...
        this.props.form.validateFields((errors, values) => {
            if(errors)return;
            this.props.action.fetchSaveCompanys(values, this.props.history);
        });
    }
    ...
    render() {
        const {getFieldProps,setFields,getFieldError} = this.props.form; //用于和表单进行双向绑定
        const {failMassage}= this.props;
        ...
        const name = getFieldProps('companyName', {
            rules: [
                {
                    required: true,
                    whitespace: true,
...
function mapStateToProps(state, props) {
    return {
        failMassage: state.businessInfoManageReducer.failMassage,
    }
}
...
export default connect(mapStateToProps, mapDispatchToProps)(Form.create()(AddBusiness));
